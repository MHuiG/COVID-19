...
import com.cov.entity.VehicleLog;
...
public class RealTimeVehicleStatJob {
    public static void main(String[] args) throws Exception {
        ...
        SparkConf conf = new SparkConf()
                    ...
                    .setMaster("local[*]")
                //  限制kafka执行的速度，每秒钟，从每一个分区最多读取多少条记录
...
//        messages.print();
    ...
    private static void calcTagert(JavaRDD<VehicleLog> vehicleRDD) {
        JavaPairRDD<String, Double> retRDD = vehicleRDD.flatMapToPair((VehicleLog vLog) -> {
        ...
        //将统计出来的结果推送到redis数据库里面
        retRDD.foreachPartition(iterator -> {
            Jedis jedis = JedisUtil.getJedis();
            while (iterator.hasNext()) {
                Tuple2<String, Double> kv = iterator.next();
                String key = kv._1;
    ...
    private static JavaPairInputDStream<String, String> createKafkaMsg(JavaStreamingContext jsc) {
        ...
        kafkaParams.put("group.id", "group");//kafka中指定消费者组
